...
The skill covers three structurally distinct audit targets. Each has its own filesystem schema and its own valid findings — a finding that is blocking in one target may be structurally impossible in another. The skill auto-detects which target the audit is pointed at and opens the findings report with the detected target so you can correct a misdetection.

**Target A — Profile source directory** (`~/agent-os/profiles/<name>/` or a checked-out single profile). Two valid layouts: `standards/` wrapper (canonical, what `project-install.sh` reads) or domain folders at profile root (the upstream `default` profile uses this). No `index.yml` belongs here — it is generated at project-install time.

**Target B — Project install** (a repo containing `agent-os/standards/index.yml`). The `standards/` directory here is a **merged artifact** from an inheritance chain, not a copy of any single profile. Domain folders contributed by different profiles flatten into one tree, and `index.yml` is generated by the install script. The audit verifies the install is complete and consistent, not that the merged shape matches any single profile.

**Target C — Enterprise profiles repository** (a standalone git repo containing multiple named profiles, typically cloned into `~/agent-os/profiles/`). Two valid layouts: a `profiles/` wrapper at repo root, or profile dirs directly at repo root. Each profile inside follows Target A schema. No `index.yml` and no `.claude/commands/agent-os/` belong here.

A **standards quality lens** applies inside any target: line-by-line review of individual standard files against the quality bar — does the standard lead with the rule, include a code example, and document something non-obvious? Or does it restate framework defaults and waste context-window tokens?

When an inheritance chain is declared in `config.yml`, the audit also produces an **inheritance coherence** section. It walks the chain end-to-end, builds a contribution map showing which profile contributes each standard and where it's overridden, and flags generality leaks (a parent contains content too specific for its level), override saturation (a parent's file is overridden by every descendant), and cross-level conflicts (a parent rule a child must contradict). These are concerns the file-by-file override mechanism otherwise hides.
